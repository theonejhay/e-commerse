...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item) {
        if ($item['quantity'] > 1) {
            // Update query to reduce quantity by 1
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                echo json_encode(["message" => "Item quantity reduced by 1"]);
            } else {
                http_response_code(500);
            ...
            // Delete query to remove item if quantity is 1
            $deleteQuery = 'DELETE FROM saved_cart WHERE id = ? AND customer_id = ?';
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                http_response_code(500);
                echo json_encode(["message" => "Failed to delete item: " . mysqli_stmt_error($stmt)]);
            }
            mysqli_stmt_close($stmt);
        }
    } else {
