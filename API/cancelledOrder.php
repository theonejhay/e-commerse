...
mysqli_stmt_store_result($fetchAddonsStmt); // Store the result to avoid "out of sync" error
...
// Prepare the insert statement for cancelled_addons
...
mysqli_stmt_close($deleteAddonsStmt);
        ...
        if (mysqli_stmt_affected_rows($deleteOrderStmt) > 0) {
            ...
            if ($contact_no) {
                $message = "Your order with Order ID $order_id has been canceled for $remarks reason. Please contact us if you have any questions.";
                ...
                echo $sms_result ? 
                    json_encode(["message" => "Order cancelled, saved to admincancelled_orders, and SMS sent successfully"]) : 
                    json_encode(["message" => "Order cancelled, saved to admincancelled_orders, but SMS failed"]);
            } else {
                echo json_encode(["message" => "Order cancelled and saved to admincancelled_orders successfully, but no contact info available"]);
            }
        } else {
